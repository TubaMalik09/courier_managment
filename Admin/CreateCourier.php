...
<head>
    ...
    <script>
        // JavaScript to show weight input field when "By Weight" is selected
        function toggleWeightInput() {
            var serviceType = document.getElementById("service_type").value;
            var weightInput = document.getElementById("weight_input");
            
            // Debugging to check if function triggers
            console.log(serviceType); // Add this for debugging
            
            if (serviceType === "by_weight") {
                weightInput.style.display = "block"; // Show weight input
            } else {
                weightInput.style.display = "none"; // Hide weight input
            }
        }
    </script>
</head>
<body>
    ...
    <form action="Recipet-courier.php" method="post">
        ...
        <div class="section">
            ...
            <select id="service_type" name="service_type" required onchange="toggleWeightInput()">
                ...
                if ($result->num_rows > 0) {
                    ...
                    while ($row = $result->fetch_assoc()) {
                        ...
                        echo "<option value='" . $serviceId . "' data-title='" . strtolower($serviceTitle) . "'>" . $serviceTitle . "</option>";
                    }
            ...
            <label >Send from Branch</label>
            <select name="Branch" required>
                <option value="">-- Select Branch --</option>

                <?php
               $sql1 = "SELECT id, Branchname FROM branch";
               $result1 = mysqli_query($conn, $sql1);
               
               if (mysqli_num_rows($result1) > 0) {
                   while ($row1 = mysqli_fetch_assoc($result1)) {
                       echo "<option value='" . $row1['id'] . "'>" . htmlspecialchars($row1['Branchname']) . "</option>";
                   }
               } else {
            ...
            <div id="weight_input" class="weight-input">
                <label for="weight">Enter Parcel Weight (kg)</label>
                <input type="number" id="weight" name="weight" step="0.01" min="0" placeholder="Enter weight in kilograms">
            </div>
        </div>
